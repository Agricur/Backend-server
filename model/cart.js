...
const InsertProduct = "INSERT INTO public.cart_items_details (cart_id,product_id,quantity,price,image) VALUES ($1,$2,$3,$4,$5)";
...
const insertProduct = (cart_id,product_id,quantity,price,image) => {
    return new Promise((resolve, reject) => {
      pool.query(InsertProduct, [cart_id,product_id,quantity,price,image], (error, results) => {
        if (error) {
          reject(error);
        } else { 
          resolve(results.rows);
